...
@Component
public class HelpCommand implements Command<MessageChannel> {
  ...
  @Autowired
  public HelpCommand(@Lazy CommandRegistry<MessageChannel> commandRegistry) {
  ...
  public void execute(String commandStr, MessageChannel channel) {
    ...
    channel.sendMessage(String.format(HELP_COMMAND_RESPONSE_FORMAT, helpMessage)).queue();
  }
